...
\begin{frame}
	\frametitle{Position Control}
	Connect antenna pointing to Gpredict and Stellarium software. 	Compute sidereal time using eight 8 bits microcontroller for a test and error obtained is 0.07 s .RP2040 is a 32 bits it is expected to improve the error. Testbench using astropy and algoritms for a calculus sidereal time.   
	\begin{figure}
		\vspace{-0.7cm}
...
\begin{frame}
	\frametitle{Position Control}
	Tests for a final system controller.
	\begin{figure}

		\includegraphics[width=\textwidth,height=2.5cm]{soft_figures/test_case_end_controller.eps}
	\end{figure} 

Next steps: 

\begin{itemize}
	\item create a software diagrams (UML,state machines, etc) for a total system
	\item Talk to system management to define terminal interfaces. 
	\item Create a test for a diagrams software. 
	\item Integration systems with a mechanical movement antenna and test using a radiosource.  
	\item Create repository to share with the community(TBD)
\end{itemize}
